...
class LoyaltyService
{
    ...
    . "Hello " . ($order->customer->name ?? 'Customer') . "!\n"
    . "You redeemed {$actualPointsNeeded} points.\n"
    . "Discount: $" . number_format((float) $actualDiscount, 2) . "\n"
    . "Order: #" . ($order->order_number ?? $order->id) . "\n"
    . "Thank you for using eOrder."
);
...
$availableBefore = max(0, (int) $customerStamp->stamps_earned - (int) $customerStamp->stamps_redeemed);
...
$availableAfter = max(0, (int) $customerStamp->stamps_earned - (int) $customerStamp->stamps_redeemed);
...
if ($rewardsAfter > $rewardsBefore) {
    ...
    "⭐ Stamp Earned\n\n"
    . "👤 Hello " . ($order->customer->name ?? 'Customer') . ",\n\n"
    . "✅ You earned a new loyalty stamp.\n"
    . "🧾 Order: #" . ($order->order_number ?? $order->id) . "\n\n"
    . "💜 Thank you for using eOrder."
);
}

foreach ($rewardAvailableAlerts as $alert) {
    $this->sendTelegramLoyaltyAlert(
        $order->customer_id,
        "🎉 Reward Available\n\n"
        . "👤 Hello " . ($order->customer->name ?? 'Customer') . ",\n\n"
        . "🏆 Congratulations! Your loyalty reward is now available.\n"
        . "🎁 Reward: " . $alert['reward'] . "\n"
        . "⭐ Stamps: " . $alert['available'] . "/" . $alert['required'] . "\n"
        . "🧾 Order: #" . ($order->order_number ?? $order->id) . "\n\n"
        . "📱 Please show your loyalty card to the cashier."
    );
}            return $stampsEarned;
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    . "Hello " . ($order->customer->name ?? 'Customer') . "!\n"
    . "You redeemed your stamp reward.\n"
    . "Reward: " . str_replace('_', ' ', ucfirst($stampRule->reward_type ?? 'Reward')) . "\n"
    . "Order: #" . ($order->order_number ?? $order->id) . "\n"
    . "Thank you for using eOrder."
);
	...
	private function sendTelegramLoyaltyAlert($customerId, string $message): void
{
    try {
        ...
        if (empty($token)) {
            Log::warning('Telegram loyalty alert skipped: bot token missing.');
            return;
        }
        ...
        if (!$response->successful()) {
            Log::warning('Telegram loyalty alert failed', [
                'customer_id' => $customerId,
        ...
        Log::error('Telegram loyalty alert exception', [
            'customer_id' => $customerId,
            'error' => $e->getMessage(),
        ]);
    }
