<div>
    <form wire:submit="submitForm">
        ...
        <div class="space-y-4">
            <div>
                <x-label for="name" value="{{ __('hotel::modules.ratePlan.name') }}" required/>
        ...
        <div class="flex w-full pb-4 space-x-4 mt-6 rtl:space-x-reverse">
            <x-button type="submit" wire:loading.attr="disabled">{{ __('hotel::modules.ratePlan.save') }}</x-button>
            <x-button-cancel type="button" wire:click="$dispatch('hideAddRatePlan')" wire:loading.attr="disabled">{{ __('hotel::modules.ratePlan.cancel') }}</x-button-cancel>
        </div>
    </form>
</div>
