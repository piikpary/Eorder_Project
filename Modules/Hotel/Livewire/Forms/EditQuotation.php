...
class EditQuotation extends Component
{
    ...
    public function mount($quotationId = null): void
    {
        ...
        $this->check_in_date = $quotation->check_in_date->format('Y-m-d');
        $this->check_out_date = $quotation->check_out_date->format('Y-m-d');
        $this->check_in_time = $quotation->check_in_time ? Carbon::parse($quotation->check_in_time)->format('H:i') : '14:00';
        $this->check_out_time = $quotation->check_out_time ? Carbon::parse($quotation->check_out_time)->format('H:i') : '11:00';
        $this->adults = $quotation->adults;
        $this->children = $quotation->children ?? 0;
        $this->rate_plan_id = $quotation->rate_plan_id;
        $this->status = $quotation->status?->value ?? QuotationStatus::DRAFT->value;
        $this->special_requests = $quotation->special_requests;
        $this->advance_paid = $quotation->advance_paid ?? 0;
        $this->advance_payment_method = $quotation->advance_payment_method;

        $this->reason_for_trip = $quotation->reason_for_trip;
        $this->means_of_transport = $quotation->means_of_transport;
        $this->place_of_origin = $quotation->place_of_origin;
        $this->vehicle_registration_number = $quotation->vehicle_registration_number;
        $this->final_destination = $quotation->final_destination;

        $this->discount_type = $quotation->discount_type ?? 'percentage';
        $this->discount_value = $quotation->discount_value ?? 0;
        $this->apply_discount = (float) ($quotation->discount_value ?? 0) > 0;
        $pivotTaxIds = $quotation->taxes->pluck('id')->map(fn ($id) => (int) $id)->values()->all();
        $this->tax_ids = !empty($pivotTaxIds)
            ? $pivotTaxIds
            : ($quotation->tax_id ? [(int) $quotation->tax_id] : []);
        $this->tax_id = $this->tax_ids[0] ?? null;

        $primary = $quotation->primaryGuest;
        if ($primary) {
            $this->guests[] = [
                'id' => $primary->id,
    ...
    public function getAmountAfterDiscountProperty(): float
    {
    ...
    public function getRoomsTotalProperty(): float
    {
        $nights = $this->check_in_date && $this->check_out_date
            ? max(1, Carbon::parse($this->check_in_date)->diffInDays(Carbon::parse($this->check_out_date)))
            : 1;
    ...
    public function submitForm()
    {
        ...
        $quotation->quotationGuests()->delete();
        ...
        $quotation->quotationExtras()->delete();
        foreach ($this->selectedExtras ?? [] as $extraId => $extraData) {
        ...
        $this->alert('success', __('hotel::modules.quotation.quotationUpdated'), [
            'toast' => true,
            'position' => 'top-end',
            'showCancelButton' => false,
            'cancelButtonText' => __('app.close'),
    ...
    {
        return view('hotel::livewire.forms.edit-quotation', [
