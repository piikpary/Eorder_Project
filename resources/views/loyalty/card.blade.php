...
<body class="text-slate-900">
    <div class="phone-shell px-4 py-5 pb-8">
        ...
        @forelse($progress as $item)
            @php
                $requiredStamps = max(1, (int) ($item['required'] ?? 5));
                $availableStamps = max(0, (int) ($item['current'] ?? 0));

                $rewardAvailable = $availableStamps >= $requiredStamps;
                $cycleStamps = $availableStamps % $requiredStamps;

                if ($rewardAvailable && $cycleStamps === 0) {
                    $cycleStamps = $requiredStamps;
                }

                $remainingStamps = max(0, $requiredStamps - $cycleStamps);
            @endphp
            ...
            <div class="loyalty-card relative mt-5 overflow-hidden rounded-[30px] p-5 shadow-2xl">
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex flex-wrap justify-center gap-3">
                        @for($i = 1; $i <= $requiredStamps; $i++)
                            <div class="stamp-box {{ $i <= $cycleStamps ? 'stamp-filled' : '' }}">
                                @if($i <= $cycleStamps)
                                    ✓
                                @endif
                            </div>
                    ...
                    @else
                        <div class="mt-5 rounded-2xl border border-purple-100 bg-white/85 p-3 text-center shadow-sm">
                            <p class="text-sm font-black text-purple-700">
                                Need {{ $remainingStamps }} more stamp{{ $remainingStamps > 1 ? 's' : '' }} to get reward.
                            </p>
                        </div>
                    @endif
