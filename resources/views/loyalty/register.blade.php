...
<html>
<head>
    <title>Create Loyalty Card</title>
...
<body class="min-h-screen bg-slate-950 text-white flex items-center justify-center p-4">
    <div class="w-full max-w-md rounded-3xl bg-slate-900 border border-slate-700 p-6 shadow-2xl">
        <h1 class="text-2xl font-bold text-center">Create Loyalty Card</h1>

        <p class="text-sm text-slate-400 text-center mt-2">
            Register your phone number to collect stamps and rewards.
        </p>
        ...
        <form method="POST" action="{{ route('loyalty.store') }}" class="mt-6 space-y-4">
            @csrf

            <input type="hidden" name="restaurant_id" value="{{ $restaurantId }}">

            <div>
                <label class="text-sm text-slate-300">Customer Name</label>
                <input
                    type="text"
                    name="name"
                    value="{{ old('name') }}"
            ...
            <a href="{{ route('loyalty.login', ['restaurant_id' => $restaurantId]) }}" class="block text-center text-sm text-slate-400 hover:text-white">
