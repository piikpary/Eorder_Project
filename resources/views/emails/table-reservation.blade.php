...
**{{ __('app.date') }}**: {{ $reservation->reservation_date_time->translatedFormat(($settings->date_format ?? dateFormat()) . ' (l)') }}

**{{ __('app.time') }}**: {{ $reservation->reservation_date_time->translatedFormat($settings->time_format ?? timeFormat()) }}
