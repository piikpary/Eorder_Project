<div class="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-gray-900 dark:border-gray-700 dark:shadow-gray-700/70 p-3 space-y-3 min-h-0">
    <div class="flex flex-col sm:flex-row sm:justify-between gap-2">
        <div class="text-base font-semibold text-gray-800 dark:text-white flex items-center gap-2 flex-wrap">
            ...
            <div class="flex items-center gap-2">
                <div @class([
                    'min-w-[4.5rem] rounded-md px-3 py-2 text-center tracking-wide',
                    'bg-skin-base/[0.2] text-skin-base' => ! is_null($reservation->table_id),
                    'bg-yellow-200 text-yellow-800' => is_null($reservation->table_id),
                ])>
                ...
                <x-secondary-button wire:click="changeTable({{ $reservation->id }})" wire:key='reservation-table-change-{{ $reservation->id }}' class="text-xs px-0.5 py-0.5 rounded">
                    <svg fill="currentColor" class="w-2.5 h-2.5 transition duration-75 group-hover:text-gray-900 dark:text-gray-200 dark:group-hover:text-white mr-1" viewBox="0 0 24 24">
            ...
            <x-secondary-button wire:click="assignTable({{ $reservation->id }})"  wire:key='reservation-table-assign-{{ $reservation->id }}' class="text-xs sm:text-sm">
        ...
        <div class="text-gray-700 dark:text-gray-400 flex flex-col space-y-1 sm:space-y-2 text-right">
            ...
            <div class="inline-flex gap-2 items-center text-xs sm:text-sm font-medium text-skin-base justify-end">
                ...
                <span class="text-xs sm:text-sm">{{ $reservation->reservation_date_time->format(dateFormat() . ' ' . timeFormat()) }}</span>
            </div>
        </div>
    </div>
    ...
    @if ($tableReservation)
    <x-dialog-modal wire:model.live="showTableModal" maxWidth="3xl">
        <x-slot name="title">
            @if ($tableReservation->table_id)
                @lang('modules.reservation.changeTable')
            @else
